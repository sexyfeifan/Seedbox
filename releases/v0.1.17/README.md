...
1. 平台小圆标来源升级为站点 icon：服务端新增站点 icon 抓取与缓存（优先站点 favicon/apple-touch-icon）
2. 站点 icon 可复用：新增 `GET /v1/items/:itemId/site-icon`，列表与详情统一复用
3. macOS Icons 回退能力：可选 `MACOSICONS_API_KEY`，无站点图标时尝试搜索回退
4. 详情页精简：移除地址来源显示、移除“返回浏览页/归档”按钮，仅保留“编辑”入口
5. 详情媒体区改为九宫格样式，与首页视觉一致，不再额外标注图片/视频文案
6. 灵感笔记改为纯内容输入，新增后在卡片角标显示最近编辑时间
